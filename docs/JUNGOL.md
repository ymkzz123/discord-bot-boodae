...
- `/user handle:<이름>`: 공개 제출 목록에서 확인되는 일치 계정 링크
...
- 정올의 브라우저용 공개 SSR 페이지만 사용하고 일시적인 403·5xx 응답은 한 번만 재시도
- 사이트 오류 페이지를 빈 검색 결과로 오인하지 않음

Jungol의 HTML과 서버 렌더링 직렬화 구조는 공식 API 계약이 아니므로 변경될 수 있습니다. 태그 결과는 공개 문제 페이지의 직렬화된 목록을, 유저는 공개 제출 페이지의 계정 요약을 읽습니다. 구조를 읽지 못하면 임의의 결과를 만들지 않고 `JUNGOL_INVALID_RESPONSE` 또는 `JUNGOL_UPSTREAM_FAILED`로 종료합니다.
